...
export function bindNavPagesCheckRightEdge(mediaQuery) {
    $('.navPages-item').hover(event => {
        ...
        $hoverEl.children('.navPage-subMenu').each((i, submenu) => {
            ...
            const $parent = $hoverEl[0].parentElement;
            $submenu
                .css('width', `${$parent.clientWidth}`);
            $(window).on('resize', () => {
                $submenu.css('width', `${$parent.clientWidth}`);
            });

            if ($submenu.offset().left !== $hoverEl[0].parentElement.getBoundingClientRect().left) {
                $submenu
                    .addClass('toLeft')
                    .css('left', `-${$submenu.offset().left - $hoverEl[0].parentElement.getBoundingClientRect().left}px`);
            }
        });
    });
